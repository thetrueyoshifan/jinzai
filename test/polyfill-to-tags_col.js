(async () => {
    ...
    const baseSelect = `SELECT eid FROM kanmi_records WHERE tags IS NULL`
    const selectTags = `SELECT eid, GROUP_CONCAT(type,'/',rating,'/',name SEPARATOR '; ') AS tags FROM sequenzia_index_matches, sequenzia_index_tags WHERE (sequenzia_index_tags.id = sequenzia_index_matches.tag) GROUP BY eid`
